...
def usage():
    print("Join a cluster: eks join <master>:<port>/<token>")
...
def update_flannel(etcd, master_ip, master_port, token):
    ...
    subprocess.check_call("snapctl restart eks.daemon-flanneld".split())
...
def update_kubeproxy(token, ca, master_ip, api_port, hostname_override):
    ...
    subprocess.check_call("snapctl restart eks.daemon-proxy".split())
...
def update_kubelet(token, ca, master_ip, api_port):
    ...
    subprocess.check_call("snapctl restart eks.daemon-kubelet".split())
...
def mark_cluster_node():
    ...
    for service in services:
        subprocess.check_call("snapctl restart eks.daemon-{}".format(service).split())
...
def reset_current_dqlite_installation():
    ...
    subprocess.check_call("snapctl stop eks.daemon-apiserver".split())
    time.sleep(10)
    ...
    subprocess.check_call("snapctl start eks.daemon-apiserver".split())
    ...
    try:
        ...
        if node_ep and force:
            ...
            print(
                ...
                "Please, run first 'eks leave' on the departing node. \n"
