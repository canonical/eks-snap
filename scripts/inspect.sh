...
function print_help {
  ...
  printf -- 'This script will inspect your eks installation. It will report any issue it finds,\n';
  printf -- 'and create a tarball of logs and traces which can be attached to an issue filed against\n';
  printf -- 'the eks project.\n';
}
...
function store_kubernetes_info {
  ...
  sudo -E /snap/bin/eks kubectl version 2>&1 | sudo tee $INSPECT_DUMP/k8s/version > /dev/null
  sudo -E /snap/bin/eks kubectl cluster-info 2>&1 | sudo tee $INSPECT_DUMP/k8s/cluster-info > /dev/null
  sudo -E /snap/bin/eks kubectl cluster-info dump 2>&1 | sudo tee $INSPECT_DUMP/k8s/cluster-info-dump > /dev/null
  sudo -E /snap/bin/eks kubectl get all --all-namespaces -o wide 2>&1 | sudo tee $INSPECT_DUMP/k8s/get-all > /dev/null
  sudo -E /snap/bin/eks kubectl get pv 2>&1 | sudo tee $INSPECT_DUMP/k8s/get-pv > /dev/null # 2>&1 redirects stderr and stdout to /dev/null if no resources found
  sudo -E /snap/bin/eks kubectl get pvc 2>&1 | sudo tee $INSPECT_DUMP/k8s/get-pvc > /dev/null # 2>&1 redirects stderr and stdout to /dev/null if no resources found
}
...
function suggest_fixes {
  # Propose fixes
  printf '\n'
  if ! systemctl status snap.eks.daemon-apiserver &> /dev/null
  then
  ...
  # node name
  nodename="$(hostname)"
  if [[ "$nodename" =~ [A-Z|_] ]] && ! grep -e "hostname-override" /var/snap/eks/current/args/kubelet &> /dev/null
  then
...
function check_certificates {
  exp_date_str="$(openssl x509 -enddate -noout -in /var/snap/eks/current/certs/ca.crt | cut -d= -f 2)"
  ...
  then
    ...
    printf -- 'Either redeploy MicroK8s or attempt a refresh with "eks refresh-certs"\n'
  fi
}
...
check_service "snap.eks.daemon-cluster-agent"
check_service "snap.eks.daemon-containerd"
check_service "snap.eks.daemon-apiserver"
check_service "snap.eks.daemon-apiserver-kicker"
check_service "snap.eks.daemon-control-plane-kicker"
check_service "snap.eks.daemon-proxy"
check_service "snap.eks.daemon-kubelet"
check_service "snap.eks.daemon-scheduler"
check_service "snap.eks.daemon-controller-manager"
